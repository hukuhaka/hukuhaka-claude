...
| Situation | Skill | Note |
|-----------|-------|------|
| Complex task/review | `/codex` | `--context` recommended |
| Bug location tracking | `/bug-finder` | Find-only, no fixes |
| Long output summary | `/summarize` | Token-efficient |
| PDF processing | `/pdf` | Extract, merge, create |
| New skill creation | `/skill-creator` | Step-by-step guide |

## Project Mapper

| Command | Description |
|---------|-------------|
| `map init` | Create empty .claude/ template |
| `map sync .` | Index + analyze + generate docs |
| `map full-sync .` | sync + scatter + validate |
| `query [question]` | Ask about project |
| `review pr [n]` | Code review with context |

Usage: `/project-mapper:[command]`

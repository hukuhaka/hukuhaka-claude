...
1. Add the server that requires authentication (for example):
...
To add an MCP server from JSON:
...
To import servers from Claude Desktop:
...
To list available resources, type `@` in your prompt to see available resources from all connected MCP servers. Resources appear alongside files in the autocomplete menu.

To reference a specific resource, use the format `@server:protocol://resource/path`:
...
To discover available prompts, type `/` to see all available commands, including those from MCP servers. MCP prompts appear with the format `/mcp__servername__promptname`.

To execute a prompt without arguments:

```
> /mcp__github__list_prs
```

To execute a prompt with arguments, pass them space-separated after the command:

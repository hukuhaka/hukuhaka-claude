...
<!-- project-local skill. TEAM-* eval scenarios require this skill to be installed under ~/.claude/skills/hukuhaka-team/ or available to the test harness. -->
...
Use TaskCreate to register outcome-focused work items for each teammate (subject = the result the teammate must deliver, not the activity). When work begins, the owning teammate flips status via TaskUpdate to `in_progress`; on completion, to `completed`. The lead uses TaskList to monitor without DMing every teammate.

- Subjects must be outcome phrases ("Auth flow returns 200 on valid creds"), not activity phrases ("write auth code")
- One task per discrete deliverable — do not bundle multiple files into one task
- Lead does not own implementation tasks; lead may create tasks but flips them only when delegating review work to self
